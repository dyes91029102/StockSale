<?php
    ...
    //取得欄位內容
    $sql = "SELECT a.PurOder_id, Reform_id, Emp_name, a.Reform_Emp_id, PurOder_date, 
            CASE WHEN Reform_date IS NULL 
            THEN  '待退貨'
            ELSE  '已退貨'
            END FROM Reform a
            JOIN PurOder b ON a.PurOder_id = b.PurOder_id
            LEFT JOIN Emp c  ON a.Reform_Emp_id = c.Emp_id
            ORDER BY 1 ";
    $st = $db->prepare($sql); 
    $st->execute();
    ...
    $Smarty->display('smarty/templates/show_return4-3.tpl');
?>